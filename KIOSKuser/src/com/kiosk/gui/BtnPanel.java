...
public class BtnPanel extends JPanel {
	...
	public BtnPanel(MainFrame mainFrame) {
		...
		btn_allCancle.addActionListener(new ActionListener() {
			
			@Override
			public void actionPerformed(ActionEvent e) {
				int result = JOptionPane.showConfirmDialog(mainFrame, "선택한 메뉴들을 모두 삭제하시겠습니까?");
				if(result == JOptionPane.OK_OPTION) {
			
					mainFrame.myWin.cartPanel.cartContainer.cartList.removeAll(mainFrame.myWin.cartPanel.cartContainer.cartList);
	...
	public void salesRegistResponseResult(JSONObject jsonObject) {
		if((boolean) jsonObject.get("result")) {
			...
			orderDetailList2 = (List<OrderDetail>) jsonObject.get("orderDetailList");
			System.out.println("여기는 response메서드 // orderDetail 사이즈 : "+orderDetailList2.size());
			mainFrame.myWin.receipt.setVisible(false);
			mainFrame.myWin.receipt.fillReceipt(orderDetailList2);
